...
public class GoldenDatasetTest {
    ...
    void golden_dataset_produces_stable_topk_and_invariants() {
        ...
        for (int i = 0; i < 20; i++) {
            ...
            int markCount = 1 + rnd.nextInt(4);
            List<Long> ids = new ArrayList<>();
            for (int j = 0; j < markCount; j++) ids.add(TestBuilders.mark((long) (1 + rnd.nextInt(5))));
            markIdsByEvidenceId.put(eid, ids);
        ...
        var result = aggregator.aggregate(Collections.unmodifiableList(candidates), markIdsByEvidenceId, 10, 0);

        AggregationAssertions.assertValidAggregation(result);

        var result2 = aggregator.aggregate(Collections.unmodifiableList(candidates), markIdsByEvidenceId, 10, 0);

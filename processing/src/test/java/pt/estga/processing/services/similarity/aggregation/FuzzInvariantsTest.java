...
public class FuzzInvariantsTest {
    ...
    void fuzz_randomized_inputs_respect_invariants_and_determinism_for_seed() {
        ...
        for (int i = 0; i < evidCount; i++) {
            ...
            markIdsByEvidenceId.put(eid, mlist);
            ...
            for (int e = 0; e < entries; e++) {
                ...
                candidates.add(new CandidateEvidence(eid, occ, sim == null ? Double.NaN : sim));
            }
        }

        var r1 = aggregator.aggregate(candidates, markIdsByEvidenceId, 20, 0);
        AggregationAssertions.assertValidAggregation(r1);

        var r2 = aggregator.aggregate(candidates, markIdsByEvidenceId, 20, 0);
